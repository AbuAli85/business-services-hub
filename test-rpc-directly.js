...
async function testRPCDirectly() {
  ...
  try {
    ...
    // Test calculate_booking_progress RPC
    ...
    const { data: milestoneResult, error: milestoneError } = await supabase
      .rpc('add_milestone', {
        ...
        title: 'Test Milestone via RPC',
        description: 'Test milestone created via RPC function',
        status: 'pending',
        weight: 1.0
      })
    ...
    if (milestoneError) {
      ...
      if (taskError) {
        ...
        const { error: deleteError } = await supabase
          .rpc('delete_milestone', { milestone_id: milestoneResult })
    ...
    console.log('\n' + '='.repeat(50))
    console.log('🎯 RPC DIRECT TEST COMPLETE')
    console.log('='.repeat(50))
