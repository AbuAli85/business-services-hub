-- Comprehensive fix for all missing columns in booking views
-- This includes all commonly used columns from the bookings table

-- Drop and recreate both views with ALL columns
DROP VIEW IF EXISTS public.booking_list_optimized CASCADE;
DROP VIEW IF EXISTS public.v_booking_status CASCADE;

-- 1. Create booking_list_optimized view with ALL columns
...
-- 2. Create v_booking_status view with ALL columns
...
SELECT
  ...
  -- ALL timestamp and location columns
  b.created_at,
  b.updated_at,
  b.due_at,
  b.scheduled_date,
...
-- Test both views
SELECT '✅ All booking views created with ALL columns!' as status;
