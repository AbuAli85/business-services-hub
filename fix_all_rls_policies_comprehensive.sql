...
-- This script fixes all RLS policies to prevent 406 errors and infinite recursion
...
-- Allow viewing profiles for business operations
CREATE POLICY "Users can view profiles for business" ON public.profiles
    FOR SELECT TO authenticated
    USING (
        -- Own profile
        auth.uid() = id OR
        -- Verified providers (for service discovery)
        (role = 'provider' AND is_verified = true) OR
        -- Profiles in same company
        EXISTS (
...
BEGIN
    ...
    RAISE NOTICE 'All RLS policies fixed successfully - no 406 errors or infinite recursion detected!';
EXCEPTION
    WHEN OTHERS THEN
...
COMMENT ON POLICY "Users can view profiles for business" ON public.profiles IS 'Allows users to view profiles for business operations (bookings, company, verified providers)';
