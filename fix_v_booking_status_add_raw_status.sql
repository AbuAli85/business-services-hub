-- Add missing raw_status column to v_booking_status view

-- Drop and recreate v_booking_status view with raw_status column
DROP VIEW IF EXISTS public.v_booking_status CASCADE;

-- Create the v_booking_status view with raw_status column added
...
SELECT 
    ...
    b.status as raw_status,  -- ADD THIS MISSING COLUMN (alias for status)
...
-- Add comment
COMMENT ON VIEW public.v_booking_status IS 'Booking status with raw_status column added - FIXED';

-- Test the view
SELECT '✅ v_booking_status view fixed with raw_status column!' as status;
SELECT COUNT(*) as total_bookings FROM public.v_booking_status;

-- Show sample data to verify raw_status column exists
SELECT 
    booking_id,
    service_title,
    client_name,
