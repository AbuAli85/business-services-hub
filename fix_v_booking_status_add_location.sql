-- Add missing location column to v_booking_status view

-- Drop and recreate v_booking_status view with location column
DROP VIEW IF EXISTS public.v_booking_status CASCADE;

-- Create the v_booking_status view with location column added
...
SELECT 
    ...
    b.requirements,
    b.notes,
    b.location,  -- ✅ ADD THIS MISSING COLUMN
    b.package_id,
...
-- Add comment
COMMENT ON VIEW public.v_booking_status IS 'Booking status with location column added - FIXED';

-- Test the view
SELECT '✅ v_booking_status view fixed with location column!' as status;
SELECT COUNT(*) as total_bookings FROM public.v_booking_status;

-- Show sample data to verify location column exists
SELECT 
    booking_id,
    service_title,
    client_name,
