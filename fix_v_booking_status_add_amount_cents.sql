-- Add missing amount_cents column to v_booking_status view

-- Drop and recreate v_booking_status view with amount_cents column
DROP VIEW IF EXISTS public.v_booking_status CASCADE;

-- Create the v_booking_status view with amount_cents column added
...
SELECT 
    ...
    b.payment_status,
    -- Invoice information
    COALESCE(i.status, 'not_created') as invoice_status,
    i.id as invoice_id,
    ...
    b.total_amount,
    b.currency,
    b.amount_cents,  -- ADD THIS MISSING COLUMN
    -- Convert amount to cents if needed
    CASE 
        WHEN b.amount_cents IS NOT NULL THEN b.amount_cents
        WHEN b.total_amount IS NOT NULL THEN (b.total_amount * 100)::bigint
        ELSE 0
...
-- Join with invoices to get invoice information
LEFT JOIN public.invoices i ON b.id = i.booking_id
-- Join with milestones to get milestone counts
LEFT JOIN (
    SELECT 
        booking_id,
...
-- Add comment
COMMENT ON VIEW public.v_booking_status IS 'Booking status with amount_cents column added - FIXED';

-- Test the view
SELECT '✅ v_booking_status view fixed with amount_cents column!' as status;
SELECT COUNT(*) as total_bookings FROM public.v_booking_status;

-- Show sample data to verify amount_cents column exists
SELECT 
    booking_id,
    service_title,
    client_name,
