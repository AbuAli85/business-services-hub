-- Add missing invoice_id column to v_booking_status view

-- Drop and recreate v_booking_status view with invoice_id column
DROP VIEW IF EXISTS public.v_booking_status CASCADE;

-- Create the v_booking_status view with invoice_id column added
...
SELECT 
    ...
    i.id as invoice_id,  -- ADD THIS MISSING COLUMN
...
-- Add comment
COMMENT ON VIEW public.v_booking_status IS 'Booking status with invoice_id column added - FIXED';

-- Test the view
SELECT '✅ v_booking_status view fixed with invoice_id column!' as status;
SELECT COUNT(*) as total_bookings FROM public.v_booking_status;

-- Show sample data to verify invoice_id column exists
SELECT 
    booking_id,
    service_title,
    client_name,
