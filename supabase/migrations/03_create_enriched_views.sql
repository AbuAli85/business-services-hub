-- Migration: Create enriched views for better data access
...
LEFT JOIN public.user_roles_v2 ur_all ON ur_all.user_id = p.id
LEFT JOIN public.roles_v2 r ON ur_all.role_id = r.id
...
CREATE OR REPLACE FUNCTION public.get_services_for_user_v2(
  user_uuid UUID,
  user_role TEXT,
  limit_count INTEGER DEFAULT 50,
  offset_count INTEGER DEFAULT 0
...
GRANT EXECUTE ON FUNCTION public.get_bookings_for_user_v2(UUID, TEXT, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_services_for_user_v2(UUID, TEXT, INTEGER, INTEGER) TO authenticated;
