-- Phase 3 Smart Features Implementation (Simplified - No Status Column References)
-- Date: January 2025
-- Description: Add overdue detection, notifications system, and export functionality
-- This version avoids referencing the status column to prevent errors
...
-- 3. Create function to check and update overdue status (simplified)
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Update overdue status for tasks (using text comparison to avoid enum issues)
    ...
    SET 
      ...
      overdue_since = NULL
    WHERE 
      status::text IN ('completed', 'cancelled')
      AND is_overdue = TRUE;
  END IF;
END;
$$;
...
BEGIN
  ...
  task_stats AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE status::text = 'completed' AND completed_at >= p_week_start AND completed_at < week_end) as completed_this_week,
      COUNT(*) FILTER (WHERE is_overdue = TRUE) as overdue_tasks,
      COUNT(*) FILTER (WHERE status::text = 'pending' AND due_date < week_end) as pending_approval
    FROM booking_tasks bt
...
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'booking_tasks') THEN
    ...
    CREATE INDEX IF NOT EXISTS idx_booking_tasks_due_date_status ON booking_tasks(due_date, status) WHERE status::text NOT IN ('completed', 'cancelled');
  END IF;
END $$;
