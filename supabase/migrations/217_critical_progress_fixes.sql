-- Critical Progress System Fixes - Migration 217
-- This migration applies essential fixes for the progress tracking system
-- while handling existing database objects gracefully

-- 1. Add missing progress_percentage column to bookings table (if not exists)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 2. Create or replace the real-time progress update function
...
-- 3. Drop and recreate the trigger for automatic progress updates
...
-- 4. Create or replace the bookings_full_view for backward compatibility
...
-- 6. Create or replace progress calculation function
