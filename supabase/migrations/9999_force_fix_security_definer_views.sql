-- Migration: Force Fix Security Definer Views (Ultimate)
-- Description: Aggressively fix all security definer views by dropping and recreating
-- Date: 2025-01-25
-- Priority: 9999 (highest priority to run absolutely last)

-- This migration uses a more aggressive approach to ensure all views are created
-- without SECURITY DEFINER property by explicitly setting SECURITY INVOKER

-- =============================================
-- 1. Force drop all views with CASCADE
-- =============================================
...
-- Recreate v_completion_analytics with explicit SECURITY INVOKER
...
-- Recreate v_revenue_forecast with explicit SECURITY INVOKER
...
-- Recreate v_booking_status with explicit SECURITY INVOKER
...
-- Recreate v_booking_status_metrics with explicit SECURITY INVOKER
...
-- Recreate v_provider_workload_analytics with explicit SECURITY INVOKER
...
-- Recreate v_booking_trends with explicit SECURITY INVOKER
...
-- Recreate v_booking_anomalies with explicit SECURITY INVOKER
...
-- Recreate v_revenue_by_status with explicit SECURITY INVOKER
...
-- Recreate v_service_performance with explicit SECURITY INVOKER
...
-- Recreate bookings_full_view with explicit SECURITY INVOKER
...
-- Recreate booking_list_optimized with explicit SECURITY INVOKER
...
-- Recreate booking_list_enhanced with explicit SECURITY INVOKER
...
COMMENT ON VIEW public.v_completion_analytics IS 'Weekly completion time analytics - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.v_revenue_forecast IS 'Revenue forecasting with trend analysis - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.v_booking_status_metrics IS 'Aggregated KPIs for the dashboard - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.v_provider_workload_analytics IS 'Provider workload and performance analytics - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.v_booking_trends IS 'Daily booking trends with counts, revenue, and progress metrics - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.v_booking_anomalies IS 'Detection of booking and revenue anomalies - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.v_revenue_by_status IS 'Revenue breakdown by booking status with 30-day trends - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.v_service_performance IS 'Service-level performance metrics and completion rates - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.bookings_full_view IS 'Complete booking information with client, provider, and service details - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.booking_list_optimized IS 'Optimized booking list for performance - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.booking_list_enhanced IS 'Enhanced booking list with additional calculated fields - FORCED RLS Compliant (Ultimate)';
COMMENT ON VIEW public.v_booking_status IS 'Booking status with progress calculation - FORCED RLS Compliant (Ultimate)';
...
DO $$
BEGIN
    RAISE NOTICE 'ULTIMATE security definer views fix completed successfully:';
    RAISE NOTICE '- Used WITH (security_invoker = true) for all views';
    RAISE NOTICE '- All views now explicitly use SECURITY INVOKER';
    ...
    RAISE NOTICE '- This migration runs with priority 9999 to ensure it executes absolutely last';
    RAISE NOTICE '- All security definer view issues should now be resolved';
END $$;
