-- Migration: Fix Security Definer Views (Comprehensive)
-- Description: Convert all security definer views to security invoker to respect RLS policies
-- Date: 2025-01-25

-- This migration fixes the security issue where views were created with SECURITY DEFINER
-- which bypasses RLS policies. We'll recreate them without SECURITY DEFINER so they respect
...
-- Drop all existing security definer views
...
SELECT 
  ...
  COALESCE(avg_revenue * 1.1, 0) as forecast_next_month -- Simple 10% growth forecast
FROM forecast_data
ORDER BY month DESC;

-- Fix v_booking_status (needed by v_booking_status_metrics)
...
-- Grant permissions on booking views
...
COMMENT ON VIEW public.v_completion_analytics IS 'Weekly completion time analytics and progress tracking - RLS Compliant';
COMMENT ON VIEW public.v_revenue_forecast IS 'Revenue forecasting with trend analysis - RLS Compliant';
COMMENT ON VIEW public.v_booking_status_metrics IS 'Aggregated KPIs for the dashboard - RLS Compliant';
COMMENT ON VIEW public.v_provider_workload_analytics IS 'Provider workload and performance analytics - RLS Compliant';
COMMENT ON VIEW public.v_booking_trends IS 'Daily booking trends with counts, revenue, and progress metrics - RLS Compliant';
COMMENT ON VIEW public.v_booking_anomalies IS 'Detection of booking and revenue anomalies - RLS Compliant';
COMMENT ON VIEW public.v_revenue_by_status IS 'Revenue breakdown by booking status with 30-day trends - RLS Compliant';
COMMENT ON VIEW public.v_service_performance IS 'Service-level performance metrics and completion rates - RLS Compliant';
COMMENT ON VIEW public.bookings_full_view IS 'Complete booking information with client, provider, and service details - RLS Compliant';
COMMENT ON VIEW public.booking_list_optimized IS 'Optimized booking list for performance - RLS Compliant';
COMMENT ON VIEW public.booking_list_enhanced IS 'Enhanced booking list with additional calculated fields - RLS Compliant';
COMMENT ON VIEW public.v_booking_status IS 'Booking status with progress calculation - RLS Compliant';
