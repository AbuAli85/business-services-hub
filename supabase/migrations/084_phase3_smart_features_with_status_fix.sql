-- Phase 3 Smart Features Implementation (With Status Column Fix)
-- Date: January 2025
-- Description: Add overdue detection, notifications system, and export functionality
-- Fixed to ensure status column exists before using it

-- 1. First, ensure the status column exists in bookings table
DO $$
BEGIN
...
-- 2. Create notifications table for smart notifications (if not exists)
...
-- 3. Add overdue detection columns to booking_tasks (if table exists)
DO $$
BEGIN
...
-- 4. Create function to check and update overdue status
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE c.table_name = 'booking_tasks' AND c.column_name = 'due_date'
  ) AND EXISTS (
    SELECT FROM information_schema.columns c
    WHERE c.table_name = 'booking_tasks' AND c.column_name = 'status'
  ) THEN
...
-- 5. Create function to send overdue notifications
...
-- 6. Create function to send approval notifications
...
-- 7. Create function to generate weekly digest data
...
BEGIN
  ...
  progress_stats AS (
    ...
    WHERE b.status IN ('in_progress', 'confirmed', 'pending', 'draft', 'pending_payment', 'paid')
  )
...
-- 8. Create function to send weekly digest
...
-- 9. Create indexes for better performance (if tables exist)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'booking_tasks') THEN
