...
-- This migration removes duplicate functions and ensures only one correct version exists

-- 1. Drop all existing update_task functions (they have different signatures)
...
-- 2. Drop all existing update_milestone_progress functions
DROP FUNCTION IF EXISTS update_milestone_progress(uuid);

-- 3. Drop all existing calculate_booking_progress functions
DROP FUNCTION IF EXISTS calculate_booking_progress(uuid);

-- 4. Create the correct update_task function with comprehensive parameters
...
-- 5. Create the correct update_milestone_progress function
...
-- 6. Create the correct calculate_booking_progress function
...
BEGIN
  ...
  IF func_count = 3 THEN
    RAISE NOTICE 'All progress functions created successfully';
  ELSE
    RAISE NOTICE 'Warning: Expected 3 functions, found %', func_count;
  END IF;
END $$;
