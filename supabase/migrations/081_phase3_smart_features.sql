...
-- 1. Create notifications table for smart notifications
...
-- 2. Add overdue detection columns to booking_tasks
ALTER TABLE booking_tasks ADD COLUMN IF NOT EXISTS is_overdue BOOLEAN DEFAULT FALSE;
ALTER TABLE booking_tasks ADD COLUMN IF NOT EXISTS overdue_since TIMESTAMP WITH TIME ZONE;
...
BEGIN
  ...
  progress_stats AS (
    ...
    WHERE b.status IN ('in_progress', 'confirmed')
  )
  SELECT jsonb_build_object(
    ...
    'progress', (
      SELECT jsonb_build_object(
        'average', ROUND(avg_progress::numeric, 1),
...
-- 8. Create scheduled job for overdue detection (using pg_cron)
-- Note: This requires pg_cron extension to be enabled
-- SELECT cron.schedule('overdue-detection', '0 9 * * *', 'SELECT update_overdue_tasks();');
-- SELECT cron.schedule('overdue-notifications', '0 10 * * *', 'SELECT send_overdue_notifications();');
-- SELECT cron.schedule('weekly-digest', '0 8 * * 1', 'SELECT send_weekly_digest(auth.uid());');
