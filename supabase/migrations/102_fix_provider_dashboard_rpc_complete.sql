-- Complete fix for Provider Dashboard RPC functions to match actual database schema
-- This migration adds missing columns and fixes all RPC functions
...
DO $$
BEGIN
    -- Add price column if it doesn't exist (alias for base_price)
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'services' AND column_name = 'price') THEN
        ALTER TABLE public.services ADD COLUMN price NUMERIC(12,3) GENERATED ALWAYS AS (base_price) STORED;
        RAISE NOTICE 'Added price column to services';
    END IF;
    
    RAISE NOTICE 'Services table columns updated successfully';
END $$;
