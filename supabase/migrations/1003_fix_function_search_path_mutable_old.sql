...
-- This migration sets search_path to 'public' for all functions that have mutable search_path
-- This prevents potential security vulnerabilities from search_path manipulation

-- First, check which functions actually exist and have mutable search_path
-- Only alter functions that exist and don't already have search_path set

-- 1. Fix hr schema functions (only if they exist)
DO $$
BEGIN
    -- Check and fix hr.get_employee_id if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'hr' AND p.proname = 'get_employee_id') THEN
        ALTER FUNCTION hr.get_employee_id() SET search_path = 'public';
        RAISE NOTICE 'Fixed hr.get_employee_id()';
    END IF;
    
    -- Check and fix hr.is_employee if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'hr' AND p.proname = 'is_employee') THEN
        ALTER FUNCTION hr.is_employee() SET search_path = 'public';
        RAISE NOTICE 'Fixed hr.is_employee()';
    END IF;
    
    -- Check and fix hr.is_hr_admin if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'hr' AND p.proname = 'is_hr_admin') THEN
        ALTER FUNCTION hr.is_hr_admin() SET search_path = 'public';
        RAISE NOTICE 'Fixed hr.is_hr_admin()';
    END IF;
    
    -- Check and fix hr.is_hr_staff if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'hr' AND p.proname = 'is_hr_staff') THEN
        ALTER FUNCTION hr.is_hr_staff() SET search_path = 'public';
        RAISE NOTICE 'Fixed hr.is_hr_staff()';
    END IF;
    
    -- Check and fix hr.is_manager if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'hr' AND p.proname = 'is_manager') THEN
        ALTER FUNCTION hr.is_manager() SET search_path = 'public';
        RAISE NOTICE 'Fixed hr.is_manager()';
    END IF;
    
    -- Check and fix hr.is_manager_of if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'hr' AND p.proname = 'is_manager_of') THEN
        ALTER FUNCTION hr.is_manager_of() SET search_path = 'public';
        RAISE NOTICE 'Fixed hr.is_manager_of()';
    END IF;
    
    -- Check and fix hr.update_updated_at_column if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'hr' AND p.proname = 'update_updated_at_column') THEN
        ALTER FUNCTION hr.update_updated_at_column() SET search_path = 'public';
        RAISE NOTICE 'Fixed hr.update_updated_at_column()';
    END IF;
END $$;

-- 2. Fix public schema functions - Core functions (only if they exist)
DO $$
BEGIN
    -- Check and fix public.enforce_milestone_transition if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'public' AND p.proname = 'enforce_milestone_transition') THEN
        ALTER FUNCTION public.enforce_milestone_transition() SET search_path = 'public';
        RAISE NOTICE 'Fixed public.enforce_milestone_transition()';
    END IF;
    
    -- Check and fix public.log_booking_changes if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'public' AND p.proname = 'log_booking_changes') THEN
        ALTER FUNCTION public.log_booking_changes() SET search_path = 'public';
        RAISE NOTICE 'Fixed public.log_booking_changes()';
    END IF;
    
    -- Check and fix public.update_task if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'public' AND p.proname = 'update_task') THEN
        ALTER FUNCTION public.update_task() SET search_path = 'public';
        RAISE NOTICE 'Fixed public.update_task()';
    END IF;
END $$;
