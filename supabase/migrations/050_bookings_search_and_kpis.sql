-- 001_bookings_search_and_kpis.sql

-- Ensure public schema exists and set search path
CREATE SCHEMA IF NOT EXISTS public;
SET search_path TO public, pg_catalog;

-- 1) Enable fuzzy search (optional - will fallback to regular indexes if not available)
DO $$
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'pg_trgm extension creation failed: % - will use regular indexes', SQLERRM;
  END;
END $$;

-- 2) Helpful indexes (using actual column names from the schema)
...
-- Trigram indexes for fuzzy search (with fallback to regular indexes)
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_trgm') THEN
    ...
    BEGIN
      CREATE INDEX IF NOT EXISTS idx_bookings_trgm_title ON public.bookings USING gin (service_title gin_trgm_ops);
      CREATE INDEX IF NOT EXISTS idx_bookings_trgm_client ON public.bookings USING gin (client_name gin_trgm_ops);
      CREATE INDEX IF NOT EXISTS idx_bookings_trgm_provider ON public.bookings USING gin (provider_name gin_trgm_ops);
      RAISE NOTICE 'Trigram indexes created successfully';
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Trigram index creation failed: %', SQLERRM;
      ...
      RAISE NOTICE 'Fallback regular indexes created';
    END;
  ELSE
    ...
    CREATE INDEX IF NOT EXISTS idx_bookings_client ON public.bookings (client_name);
    CREATE INDEX IF NOT EXISTS idx_bookings_provider ON public.bookings (provider_name);
    RAISE NOTICE 'Regular indexes created (pg_trgm not available)';
  END IF;
END $$;

-- 3) Aggregation view for KPI header (using actual status values)
...
-- 5) Optional RPC for role-filtered/paginated listings (RLS-friendly)
-- NOTE: If you prefer direct Supabase client queries with RLS, you can skip this RPC.
...
-- You may also add SECURITY DEFINER + explicit grants if using RPC across roles.

-- 6) Create triggers to keep denormalized columns in sync
