-- Fix user_role enum - Add missing application values
-- Current enum has: admin, manager, user, viewer
-- Application needs: admin, provider, client, staff
...
-- Step 2: Show all enum values now
...
SELECT 
    ...
    CASE 
        ...
        -- Map old enum values to new ones if needed
        WHEN au.raw_user_meta_data->>'role' = 'manager' THEN 'provider'::user_role
        WHEN au.raw_user_meta_data->>'role' = 'user' THEN 'client'::user_role
        ELSE 'client'::user_role  -- Default
    END,
...
-- Step 4: Sync roles from auth to profiles
UPDATE profiles p
SET 
    role = CASE 
        WHEN au.raw_user_meta_data->>'role' = 'client' THEN 'client'::user_role
...
-- Step 5: Sync roles from profiles to auth (where auth role is missing)
...
-- Step 9: Show all users with their roles
SELECT 
    au.email,
