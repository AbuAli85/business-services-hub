-- Add missing notes column to v_booking_status view

-- Drop and recreate v_booking_status view with notes column
DROP VIEW IF EXISTS public.v_booking_status CASCADE;

-- Create the v_booking_status view with notes column added
...
SELECT 
    ...
    b.due_at,
    b.requirements,
    b.notes,  -- ADD THIS MISSING COLUMN
    b.package_id,
...
-- Add comment
COMMENT ON VIEW public.v_booking_status IS 'Booking status with notes column added - FIXED';

-- Test the view
SELECT '✅ v_booking_status view fixed with notes column!' as status;
SELECT COUNT(*) as total_bookings FROM public.v_booking_status;

-- Show sample data to verify notes column exists
SELECT 
    booking_id,
    service_title,
    client_name,
