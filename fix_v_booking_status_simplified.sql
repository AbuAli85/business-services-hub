-- Simplified fix for v_booking_status view
-- Only use columns that actually exist in the profiles table

-- Drop and recreate v_booking_status view with only existing columns
DROP VIEW IF EXISTS public.v_booking_status CASCADE;

-- Create the v_booking_status view with only existing columns
...
SELECT 
    ...
    -- Client information (using only existing columns)
    ...
    -- Use only existing address column
    COALESCE(
        client_comp.address,
        c.address,
        'Address not provided'
    ) as client_company_address,
    -- Use only existing phone column
    COALESCE(
        client_comp.phone,
        c.phone
    ) as client_company_phone,
    -- Use only existing email column
    COALESCE(
        client_comp.email,
        c.email
    ) as client_company_email,
    
    -- Provider information (using only existing columns)
...
-- Add comment
COMMENT ON VIEW public.v_booking_status IS 'Booking status with proper client company handling - SIMPLIFIED (only existing columns)';

-- Test the view
SELECT '✅ v_booking_status view fixed with simplified column handling!' as status;
