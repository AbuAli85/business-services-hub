...
export async function POST(request: NextRequest) {
  try {
    ...
    const { days_back = 30, group_by = 'day' } = body
    ...
    if (error) {
      console.error('Error calling get_booking_trends:', error)
      return jsonError(500, 'INTERNAL_ERROR', 'Failed to fetch booking trends')
    }

    return NextResponse.json(data || [])

  } catch (error) {
    console.error('Booking trends API error:', error)
