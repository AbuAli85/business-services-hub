...
if (typeof window !== 'undefined') {
  ...
  // Also check if we're stuck in a redirect state for too long (more than 30 seconds)
  const stuckRedirectTime = Date.now() - REDIRECT_TRACKER.lastRedirectTime
  const isStuckRedirect = REDIRECT_TRACKER.redirecting && stuckRedirectTime > 30000
  
  if (isPageRefresh || isStuckRedirect) {
    console.log('🔄 Page refresh or stuck redirect detected, resetting redirect tracker', {
      isPageRefresh,
      isStuckRedirect,
      stuckRedirectTime
    })
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (REDIRECT_TRACKER.redirecting || 
        ...
        isProviderLoaded) {
      console.log(`⏭️ Global redirect guard: ${timeSinceRedirect}ms since last redirect, isRedirectingInStorage: ${isRedirectingInStorage}, isProviderLoaded: ${isProviderLoaded}, skipping auth check`)
      return
    }
