-- Add missing total_milestones column to v_booking_status view

-- Drop and recreate v_booking_status view with total_milestones column
DROP VIEW IF EXISTS public.v_booking_status CASCADE;

-- Create the v_booking_status view with total_milestones column added
...
SELECT 
    ...
    -- Milestones count (ADD THIS MISSING COLUMN)
...
-- Add comment
COMMENT ON VIEW public.v_booking_status IS 'Booking status with total_milestones column added - FIXED';

-- Test the view
SELECT '✅ v_booking_status view fixed with total_milestones column!' as status;
SELECT COUNT(*) as total_bookings FROM public.v_booking_status;

-- Show sample data to verify total_milestones column exists
SELECT 
    booking_id,
    service_title,
    client_name,
