-- Fix Status Mapping: Progress 100% should show as "completed"
-- Run this in Supabase SQL Editor to fix the status display issue

-- Drop and recreate the view with corrected status mapping
...
SELECT 
  ...
  0 as total_milestones,                   -- Placeholder - will be calculated by app if needed
  0 as completed_milestones,               -- Placeholder - will be calculated by app if needed
  
  -- CORRECTED Status mapping for UI - 100% progress = completed
  CASE 
...
-- Success message
SELECT '✅ Status mapping fixed - 100% progress now shows as "completed"!' as status;
